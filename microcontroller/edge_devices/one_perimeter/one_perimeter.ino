...
// one_perimeter SETUP (IN, IN, IN, IN, IN, OUT)

String hwid = "one_perimeter";
...
void loop() {
  ...
  pin4_cumulative = pin4_cumulative + pin4_status;
  pin5_status = digitalRead(pin5);
  pin5_cumulative = pin5_cumulative + pin5_status;
  ...
  if (counter == counter_reference){
    ...
    // Store Input
    String input_url_w_data = input_url + pin1_cumulative + "z" + pin2_cumulative + "z" + pin3_cumulative + "z" + pin4_cumulative + "z" + pin5_cumulative;
    ...
    if (output_http_response_code > 0) {
      ...
      pin6_output_status = payload.substring(0,1);
